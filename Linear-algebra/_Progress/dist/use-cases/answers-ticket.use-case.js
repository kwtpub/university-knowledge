...
export async function answersTicketUseCase() {
    ...
    const ticketPairs = selectOrderedTicketPairs(ticketRepo.getAll(), FIRST_PAIR_NUMBER, LAST_PAIR_NUMBER, PAIR_OFFSET);
...
function selectOrderedTicketPairs(allTickets, firstPairNumber, lastPairNumber, offset) {
    ...
    const pairs = [];
    for (let firstNumber = firstPairNumber; firstNumber <= lastPairNumber; firstNumber += 1) {
...
function updateTicketsAnswers(allTickets, answers) {
    const answersByTicketId = new Map();
    ...
    return allTickets.map((ticket) => {
        ...
        return new TicketModel(ticket._id, ticket.numberTicket, ticket.theme, ticket.text, ticket.countAnswer + statuses.length, lastStatus);
    });
}
