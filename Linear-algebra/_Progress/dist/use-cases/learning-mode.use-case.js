...
export async function learningModeUseCase() {
    ...
    const themeTickets = allTickets.filter((ticket) => ticket.theme === selectedTheme);
...
function getUniqueThemes(tickets) {
...
function countTicketsByTheme(tickets, theme) {
...
function printTicket(ticket, current, total) {
    console.log("\n" + "=".repeat(80));
...
function saveLearningProgress(repo, answers) {
    ...
    for (const answer of answers) {
        ...
        const attemptCount = existingProgress
            ...
            : 1;
        const progress = new LearningProgressModel(existingProgress ? existingProgress._id : repo.getNextId(), answer.ticket._id, answer.ticket.numberTicket, answer.ticket.theme, answer.ticket.text, answer.status, attemptCount, now);
        repo.addOrUpdate(progress);
    }
}
async function generateLearningProgressMarkdown(repo, theme) {
    const fs = await import("fs");
    const path = await import("path");
    const { fileURLToPath } = await import("url");
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
    const outputPath = path.join(__dirname, "../../", `Прогресс_обучения_${sanitizeFileName(theme)}.md`);
...
function groupByStatus(progress) {
    const grouped = {
        [STATUS.GOOD]: [],
        [STATUS.AVERAGE]: [],
        [STATUS.BAD]: [],
...
function calculateStats(progress) {
    ...
    const good = progress.filter((p) => p.understandingStatus === STATUS.GOOD)
        .length;
    const average = progress.filter((p) => p.understandingStatus === STATUS.AVERAGE).length;
